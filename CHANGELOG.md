...
- Upgraded cache maintenance to cache v21 generation files with manifest-last commits and incremental refreshes. Changed MCP watcher refreshes now reuse unchanged file metadata, parse only new/modified files, merge old dependency sidecars, and remap BM25 postings instead of falling back to full source reindexing.
...
- Reworked warm watcher maintenance around a filesystem-event queue batched by `poll_interval_seconds`, so large edit bursts no longer scan the full source tree every tick.
- Added cache v23 outline sidecars with offset/length records, letting one-shot `codedb_outline` seek directly to one file's precomputed outline.
...
- Re-ran incremental maintenance on the `u3dclient` Unity runtime C# config after the live overlay/event-queue changes: cache-hit `codedb_status` 0.392s one-shot, adding 1,000 small `.cs` files 0.856s warm apply, modifying 1,000 files 0.811s warm apply, deleting 1,000 files 0.375s warm apply, all returning `cache: live-incremental` inside the warm path.
- Replaced README one-shot tool timings with warm in-process benchmark rows. Current `u3dclient` warm text-search samples: `codedb_text_search PoolManager` 1.041ms, `Joystick` 1.790ms, regex `class\\s+Joystick` 25.767ms; equivalent `rg` runs on the same roots and Editor exclusion were 1238.513ms, 451.176ms, and 3312.850ms.
