...
- cache v14 后 cache-hit 非语义单次内存：约 267 MB WS / 351 MB private

索引耗时：

| 场景 | Cache | 耗时 | 峰值内存 | 说明 |
|---|---|---:|---:|---|
| cache v14 临时存储冷重建 | miss | 30.819s | 1334.5 MB WS / 1357.3 MB private | scan、tree-sitter 声明解析、embedding、BM25、compact cache save |
| cache-hit index open | hit | 5.060s | 266.7 MB WS / 350.6 MB private | 包含进程启动和 cache load |
| 单次 CLI `codedb_status` | hit | 5.040s | 269.4 MB WS / 351.1 MB private | 包含进程启动和 index load；真实使用应保持 MCP 常驻 |
| 单次符号 `codedb_search PoolManager` | hit | 5.187s | 258.3 MB WS / 350.8 MB private | 符号形态 query 走 BM25 + 精确符号增强，不加载 embeddings |
| 单次 `codedb_callers PoolManager` | hit | 5.227s | 262.0 MB WS / 350.8 MB private | definition-anchored typed caller lookup |
| 单次 `codedb_deps PoolManager.cs` | hit | 5.247s | 325.4 MB WS / 377.2 MB private | dependency sidecar 从 `deps.bin` 懒加载 |
| 单次自然语言语义 `codedb_search` | hit | 5.328s | 403.8 MB WS / 453.6 MB private | 自然语言 query 才懒加载 Model2Vec 和 `embeddings.bin` |
| 单次包含 20 个符号搜索的 `codedb_bundle` | hit | 5.087s | 267.9 MB WS / 318.0 MB private | 验证 compact bundle 输出无管道 backpressure |
...
cache v14 不再常驻完整文件正文。大范围未限定的 regex 会按需读取源码文件，所以这种场景可能比 `rg` 慢；path-scoped regex、file glob、符号搜索、引用、依赖和 outline 才是 MCP 的低延迟主路径。
...
7. **内存友好缓存层**：cache v14 吸收 `justrach/codedb` 的 bounded content cache 思路：完整文件正文、chunk 预览正文、重复 chunk 文件路径、重复 language/kind 字符串、BM25 postings、embeddings、正向/反向依赖、graph 对象和 Louvain 结果都不再默认常驻。工具需要时再按需读取精确行、postings、embedding、依赖或图数据。
