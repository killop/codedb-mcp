...
`code-module-atlas` skill 会调用 `codedb_module_atlas`，把目标项目 `.codedb-mcp` 下的模块/文件图数据转换成打包在 skill 内的网页数据：
...
网页中每个点是一个代码文件；左侧是模块列表和当前模块内文件列表。选择模块会只展示该模块相关文件，选择文件会聚焦到对应节点并打开文件详情。
