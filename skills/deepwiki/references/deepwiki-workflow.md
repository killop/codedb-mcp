...
3. Use `codedb_flow` for a compact first evidence page when the user asks about an architecture area or flow.
4. Call `codedb_module_atlas` only when a broad dependency-connected module/file inventory is useful.
5. Continue from exact returned paths, symbols, dependencies, callers, callpaths, outlines, and cited snippets.
...
Use dependency and symbol evidence as the first draft:

- keep candidates with strong internal dependency cohesion and exact symbol/caller evidence;
- merge candidates when they share central symbols, entry points, or repeated dependency edges;
...
1. Start from exact paths, symbols, dependencies, callers, or a `codedb_module_atlas` inventory row and record why the files belong together.
2. Gather additional candidate files with `codedb_search`, `codedb_find`, `codedb_glob`, `codedb_deps`, and `codedb_callpath`.
